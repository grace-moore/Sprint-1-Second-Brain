    ...
    <Layout>
      <div className="space-y-8">
        ...
        {isLoading ? (
          ...
          <div className="bg-white border border-dashed border-border rounded-lg p-12 text-center" role="status">
          ...
          <ul
            ...
            aria-label="Argument review sessions"
          >
            {sessions.map((session) => (
              <li key={session.id}>
                <button
                  ...
                  aria-label={`Open session: ${session.title || "Untitled Session"}, status: ${session.status}`}
                >
                  <Card className="shadow-sm border-border hover:border-primary/30 transition-colors cursor-pointer group h-full">
                      ...
                      <CardDescription className="flex items-center text-xs">
                        ...
                        {session.rating && (
                          <span className="flex items-center ml-3 pl-3 border-l border-border text-amber-500">
                            ...
                            <span aria-label={`Rating: ${session.rating} out of 5`}>{session.rating}/5</span>
                          </span>
                        )}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      {session.analysisCount > 0 ? (
                        <dl className="flex items-center justify-start gap-6 pt-2 border-t border-border/50">
                          <div className="flex flex-col items-center">
                            <dt className="sr-only">Biases found</dt>
                            <dd className="flex items-center text-sm font-medium text-foreground">
                              <Scale className="w-4 h-4 mr-1.5 text-muted-foreground" aria-hidden="true" />
                              {session.biasCount}
                            </dd>
                          </div>
                          <div className="flex flex-col items-center">
                            <dt className="sr-only">Fallacies detected</dt>
                            <dd className="flex items-center text-sm font-medium text-foreground">
                              <Target className="w-4 h-4 mr-1.5 text-muted-foreground" aria-hidden="true" />
                              {session.fallacyCount}
                            </dd>
                          </div>
                          <div className="flex flex-col items-center">
                            <dt className="sr-only">Counter-arguments</dt>
                            <dd className="flex items-center text-sm font-medium text-foreground">
                              <ShieldAlert className="w-4 h-4 mr-1.5 text-muted-foreground" aria-hidden="true" />
